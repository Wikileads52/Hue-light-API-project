...
from app.color_control import define_color
...
def check_light_status(url):
    response = requests.get(url, headers=headers, timeout=5, verify= False)
    if response.status_code == 200:
        data = response.json()
        #Only a boolean exist in the "on" json object
...
def user_select_what_to_do(on_off, url):
    actions_available={
        "1" : "Turn light on/off",
        "2" : "Choose brightness",
        "3" : "Choose color",
    ...
    if on_off:
        ...
        if actions_selected == "1":
            ...
            new_light_state = turn_light_on_off(on_off, url)
        elif actions_selected == "2":
            print ("You chose to change brightness")
            define_brightness(url)
            return on_off
        elif actions_selected == "3":
            print ("You chose to change the color of the light")
            define_color(url)
            return on_off
        elif actions_selected == "4":
            print("You chose to quit the program")
            return None, None
        else:
            print("Invalid selection. Please choose a valid option.")
            user_select_what_to_do(on_off, url)
    else:
        new_light_state = turn_light_on_off(on_off, url)
        return new_light_state
    return actions_available
    
def turn_light_on_off(on_off, url):
    if on_off:
        body={"on":{"on": False}}
    else:
        body={"on":{"on": True}}
